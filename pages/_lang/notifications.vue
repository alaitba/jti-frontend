			...
			<div class="anketa">
						...
						<div class="list-item item" v-for="(item,key) in notifications">
							<div class="list-item__content item__content">
								<p :class="{'title': true, 'mb4': item.type == 'NotificationFromAdmin'}" v-if="item">
									<template v-if="item.type=='LeadCreated'">
										...
										<template v-else>
											{{$t('Анкета отправлена потребителю на заполнение ')}}
										</template>
									</template>								
									<template v-else-if="item.type=='LeadQualified'">
										...
										<template v-else>
											{{$t('Потребитель завершил заполнение анкеты на сайте realday.kz ')}}
										</template>										
									</template>								
									<template v-else-if="item.type=='LeadEffective'">
										{{$t('Потребитель отправил код с пачки')}}
									</template>								
									<template v-else-if="item.type=='RewardBought'">
										{{item.data.rewardTitle}}
									</template>								
									<template v-else-if="item.type=='HappyBirthday'">
										{{$t('Уважаемый Партнер, поздравляем Вас с днем рождения. С уважением, JTI')}}
									</template>								
							...
							<div class="list-item__status item__status">
								<template v-if="item.type =='RewardBought' || item.type == 'BalanceReplenished'">									
									...
									- {{item.data.price | formatAmount}} {{$t('notifications-amount')}}						
									</p>
								</template>
								<template v-else>									
									<p class="point" v-if="item.data.amount">
										+ {{item.data.amount | formatAmount}} {{$t('notifications-amount')}}			
									</p>
								</template>									
				...
				>
					<div class="anketa__head">
						...
						>
							{{$t('У вас пока нет новых уведомлений')}}
						</h3>
					</div>
