    ...
    <template v-else>
      <div class="quiz-inside">
        <div class="container" v-if="questions">
          <h3
            class="section__title section__title--link black"
            @click="$router.go(-1)"
            v-if="questions.title"
          >{{questions.title[$i18n.locale ==='kk' ? 'kz' : 'ru']}}</h3>
          <div class="questions" v-if="questions.questions.length">
            <div v-for="(item, index) in questions.questions" v-show="questionIndex == index">
              ...
              <div class="questions__index">
                <template
                  v-if="$i18n.locale === 'ru'"
                >Вопрос {{index + 1}} из {{questions.questions.length}}</template>
                <template v-if="$i18n.locale === 'kk'">Сұрақ №{{index + 1}}</template>
              </div>
              ...
              <template v-if="questions.type == 'poll' && item.type == 'choice'">
                <div class="options" v-if="item.answers">
                  <template v-for="(response, key) in item.answers">
                    <div
                      :class="{'options__item options__item--radio': true, 'active': checkArrayData(response.id, item.selected), 'options__item--photo': response.photo}"
                    >
                      <label class="checkbox-container">
                        <input type="checkbox" :value="response.id" v-model="item.selected" />
                        <img
                          :src="response.photo"
                          alt
                          class="img"
                        ...
                        <p class="title title--break-word">
                          {{
              ...
              <template v-else-if="questions.type == 'poll' && item.type == 'text'">
                <div class="form">
                  <label for class="title__label">{{$t('Введите ответ')}}</label>
                  <div class="form-group">
                    <input type="text" class="form__input" placeholder=" " v-model="item.selected" />
                    <label for="input" class="form__label">{{$t('Ваш ответ')}}</label>
                  </div>
                </div>
              </template>
              ...
              <template v-else-if="questions.type == 'quiz'">
                <div class="options" v-if="item.answers">
                  <template v-for="(response, key) in item.answers">
                    <div
                      :class="{'options__item options__item--radio': true, 'active': item.selected == response.id, 'options__item--photo': response.photo}"
                    >
                      <label class="radiobutton-container">
                        <input type="radio" :value="response.id" v-model="item.selected" />
                        <img
                          :src="response.photo"
                          alt
                          class="img"
              ...
              <div class="questions__сontrols">
                <button
                  ...
                  :disabled="item.selected.length < 1"
                  @click="next(item.id,item.selected)"
                  v-if="(questionIndex + 1)  != questions.questions.length"
                >{{$t('Следующий вопрос')}}</button>
                <button
                  class="button button--green"
                  :disabled="item.selected.length < 1"
                  @click="next(item.id, item.selected, 'finish')"
                  v-if="(questionIndex + 1)  == questions.questions.length"
                >
                  <template v-if="questions.type == 'quiz'">{{$t('Завершить викторину')}}</template>
                  <template v-if="questions.type == 'poll'">{{$t('Завершить опрос')}}</template>
                </button>
              </div>
            </div>
      ...
      try {
        ...
        if (res.total == res.correct) {
          if (this.questions.type == "quiz") {
            this.title =
              this.$t("Результат:") + res.correct + this.$t("из") + res.total;
          ...
          if (res.money_status == "ok") {
            ...
            this.text =
              this.$t("Поздравляем! Вы получите") +
              res.amount +
              this.$t("тг на баланс за успешное прохождение");
          } else if (res.money_status == "failed") {
            this.title =
              this.$t("Результат:") + res.correct + this.$t("из") + res.total;

            this.text = this.$t(
              "Произошла проблема при начислении баланса. Свяжитесь с вашим торговым агентом"
            );
          } else {
            this.text = this.$t(
              "Спасибо за прохождение опроса, Ваше мнение очень важно для нас"
            );
          }
          ...
          this.text = this.$t(
            "Вы не смогли ответить на все вопросы правильно. Попробуйте ответить еще раз"
          );
