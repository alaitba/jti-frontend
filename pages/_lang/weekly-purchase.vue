			...
			<div class="purchase">
				<div class="container">
					...
					<div class="purchase__recommendations" >
						<div class="title">
							{{$t('Рекомендуемый закуп за один визит')}}
						</div>
						...
						<template v-else>
							<div class="amount amount--text">
								{{$t('Заполните график визитов для расчета')}}
							</div>
						</template>
					</div>
					...
					<div class="purchase__update-date">
                        ...
                        <p v-if="$i18n.locale === 'kk'">
                            Мәліметтер {{selectedStore.lastUpdated |  formatData}} бойынша өзекті
                        </p>
					</div>
...
<script>
	...
	export default{
		...
		data(){
			return{
				...
				calendarConfigs: {
	                ...
	                monthNames: ["Қаңтар/Январь", "Ақпан/Февраль", "Наурыз/Март", "Сәуір/Апрель", "Мамыр/Май", "Маусым/Июнь", "Шілде/Июль", "Тамыз/Август", "Қыркүйек/Сентябрь", "Қазан/Октябрь", "Қараша/Ноябрь", "Желтоқсан/Декабрь"],
		...
		async mounted(){
            ...
            await this.setTranslateCalendar();
		...
		methods:{
            ...
            setTranslateCalendar(){
                // console.log('a', a ,  $('.vfc-top-date'))
                let elements = '<section class="vfc-dayNames kz"><span>Дс</span><span>Сс</span><span>Ср</span><span>Бс</span><span>Жм</span><span>Сб</span><span>Жс</span></section>'
                $('.vfc-top-date').after(elements)
            },
			getWeekends(){
